...
class Server(object):
    def __init__(self, device, model_func, init_model, init_par_list, datasets, method, args):
        ...
        # Proposed early stopping (client-task-vector based with EMA)
        self.proposed_early_stop_counter = 0
        self.proposed_early_stop_round = None
        self.prev_phi = None                      # previous round statistic
        self.ema_phi = None                       # EMA-smoothed phi
        self.prev_ema_phi = None                  # previous EMA phi for delta
        ...
        self.client_phi_record = np.zeros((self.args.comm_rounds, 1))
        self.client_delta_record = np.zeros((self.args.comm_rounds, 1))
        self.client_shift_record = np.zeros((self.args.comm_rounds, 1))
        self.client_dispersion_record = np.zeros((self.args.comm_rounds, 1))
    ...
    def _save_results_(self):
        ...
        # Save: test_perf (5 cols) + proposed phi + proposed delta
        final_results = np.hstack((
            self.test_perf,              # (R,5)
            self.client_phi_record,     # (R,1)
            self.client_delta_record,   # (R,1)
            self.client_shift_record,   # (R,1)
            self.client_dispersion_record  # (R,1)
        ))
    ...
    def compute_proposed_metrics(self, t, selected_clients):
        """Compute client-task-vector based delta metrics without checking early stopping condition."""
        if selected_clients is None or len(selected_clients) == 0:
            return

        eps = 1e-12

        # alpha derived from patience (no new knob)
        alpha = 2.0 / (float(self.args.patience) + 1.0)
        alpha = max(min(alpha, 1.0), 0.0)  # safety clamp

        init_global = self.init_server_model_params_list           # [D]
        local_models = self.clients_params_list[selected_clients]  # [M, D]

        # 1) centroid of selected clients
        centroid = torch.mean(local_models, dim=0)                 # [D]

        # 2) progress (init-referenced, mean over clients)
        shift = torch.norm(local_models - init_global.unsqueeze(0), p=2, dim=1).mean().item()

        # 3) dispersion around centroid
        dispersion = torch.norm(local_models - centroid.unsqueeze(0), p=2, dim=1).mean().item()

        self.client_shift_record[t] = shift
        self.client_dispersion_record[t] = dispersion
        
        # 4) raw phi
        phi = shift * (1.0 + dispersion / (shift + eps))

        # EMA(phi)
        if self.ema_phi is None:
            self.ema_phi = phi
        else:
            self.ema_phi = alpha * phi + (1.0 - alpha) * self.ema_phi

        # record EMA(phi)
        self.client_phi_record[t] = self.ema_phi

        # init for delta
        if self.prev_ema_phi is None:
            self.prev_ema_phi = self.ema_phi
            self.client_delta_record[t] = 0.0
            return 0.0, shift, dispersion

        # delta computed on EMA(phi)
        delta = abs(self.ema_phi - self.prev_ema_phi) / (abs(self.prev_ema_phi) + eps)
        self.client_delta_record[t] = delta

        # update prev EMA
        self.prev_ema_phi = self.ema_phi

        return delta, shift, dispersion

    def check_proposed_early_stopping(self, t, selected_clients):
        """Check early stopping condition based on computed metrics."""
        if selected_clients is None or len(selected_clients) == 0:
            return

        result = self.compute_proposed_metrics(t, selected_clients)
        if result is None:
            return

        delta, shift, dispersion = result

        # If already stopped, just print and return
        if self.proposed_early_stop_round is not None:
            print(
                f"shift={shift:.4f} | disp={dispersion:.4f} | "
                f"delta={delta:.4f} | "
        ...
        # patience/threshold logic
        if delta < self.args.threshold:
    ...
    def train(self):
        ...
        for t in progress_bar:
            ...
            # Proposed early stopping: compute metrics BEFORE aggregation
            if self.args.proposed:
                self.check_proposed_early_stopping(t, selected_clients)
            elif not self.args.fast:
                self.compute_proposed_metrics(t, selected_clients)
