    ...
    for method in "${methods[@]}"; do
        for seed in "${seeds[@]}"; do
            for p in "${patiences[@]}"; do
                cmd="python3 train.py --local-learning-rate 0.1 --proposed --fast --non-iid --dataset ${dataset} --num_class ${num_class} --pretrain --method ${method} --seed ${seed} --split-coef 0.1 --patience ${p} --threshold 0.1"

                JOB_NAME="fl_${dataset}_${method}_s${seed}_p_${p}_proposed"
