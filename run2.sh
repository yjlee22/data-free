...
for dataset in "${datasets[@]}"; do
    ...
    for method in "${methods[@]}"; do
        for seed in "${seeds[@]}"; do
            for c in "${coeffs[@]}"; do
                cmd="python3 train.py --local-learning-rate 0.1 --proposed --fast --non-iid --dataset ${dataset} --num_class ${num_class} --pretrain --method ${method} --seed ${seed} --split-coef ${c} --threshold 0.1"

                JOB_NAME="fl_${dataset}_${method}_s${seed}_c${c}_proposed"
